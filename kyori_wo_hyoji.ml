(* 目的: 駅名を2つ受け取り、2駅間の距離を返すを文字列を表示する *)
(* romaji_to_kanji : string -> string -> int list -> int *)
...
let rec kyori_wo_hyoji eki1 eki2 =
  ...
  let kyori = get_ekikan_kyori eki_kanji1 eki_kanji2 global_ekikan_list in
    if eki_kanji1 = "" && eki_kanji2 = "" then "～という駅は存在しません"
    else if eki_kanji1 = "" then eki1 ^ "という駅は存在しません"
    else if eki_kanji2 = "" then eki2 ^ "という駅は存在しません"
    else if kyori = infinity then eki1 ^ "駅と" ^ eki2 ^ "駅はつながっていません"
    else eki_kanji1 ^ "駅から" ^ eki_kanji2 ^ "駅までは" ^ (string_of_float kyori) ^ "kmです"

(* テスト *)
let test1 = kyori_wo_hyoji "" "" = "～という駅は存在しません"
let test2 = kyori_wo_hyoji "hakata" "nogizaka" = "hakataという駅は存在しません"
let test3 = kyori_wo_hyoji "nogizaka" "sagashi" = "sagashiという駅は存在しません"
let test4 = kyori_wo_hyoji "otemachi" "akasaka" = "otemachi駅とakasaka駅はつながっていません"
let test5 = kyori_wo_hyoji "heiwadai" "eidanakakuka" = "平和台駅から営団赤塚駅までは1.8kmです"
let test6 = kyori_wo_hyoji "eidannarimasu" "wakousi" = "営団成増駅から和光市駅までは2.1kmです"
